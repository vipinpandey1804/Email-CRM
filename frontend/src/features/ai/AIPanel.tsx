...
import { Sparkles, ShieldCheck, Pen, Target, Wand2 } from 'lucide-react'

interface Props {
  campaignId?: string
  subjectLine?: string
  emailBody?: string
  /** Title used to pre-fill the AI email builder (e.g. template name). */
  emailTitle?: string
  /** True when rendered inside the editor — enables inserting generated MJML. */
  inEditor?: boolean
}

type Tab = 'build' | 'subject' | 'spam' | 'copy' | 'cta'
...
export function AIPanel({
  ...
  const [activeTab, setActiveTab] = useState<Tab>(inEditor ? 'build' : 'subject')
  ...
  return (
    <div className="p-4">
      ...
      {/* Tabs */}
      <div className="grid grid-cols-5 gap-1 mb-4 bg-accent rounded-lg p-1">
        {tabs.map(({ id, label, icon: Icon }) => (
          <button
            key={id}
            onClick={() => setActiveTab(id)}
            className={`flex flex-col items-center py-1.5 rounded-md text-[11px] transition-colors ${
      ...
      {/* Tab content */}
      {activeTab === 'build' && <EmailBuilder defaultTitle={emailTitle} inEditor={inEditor} />}
